...
export async function add_comment(comment_data){
    const session = await mongoose.startSession();
    try{
        return await session.withTransaction(async()=>{
    const user=await get_user_by_id(comment_data.user);
    if(!user)throw notFound("user not found");
    const comment=await create_comment(comment_data,session);
    if(comment.onmodel==="Post"){
        await add_comment_to_post(comment.commentableId,comment._id,session);
    }
    else if(comment.onmodel==="Comment"){
        await add_comment_to_reply(comment.commentableId,comment._id,session);
    }
    return comment;
    })}
    finally{
...
export async function remove_comment(comment_id){
    const session =await mongoose.startSession()
    try{
         return await session.withTransaction(async()=>{
